...
function getOverviewReport(reports) {
	...
	try{
		...
		// clone objects so we don't have to worry about side effects
			summary.total.sloc += report.complexity.aggregateAverage.sloc.physical;
			summary.total.maintainability += report.complexity.maintainability;

			var aggregateAverage = _.cloneDeep(report.complexity.aggregateAverage);
			var reportItem = {};
...
function writeFileReport(outdir, report, source, options) {
	var fileSource = fs.readFileSync(fileTemplate).toString();

	if (!source) {throw new Error('No source!');}


	try {
		var parsed = _.template(fileSource)({
