...
$(function() {
	...
	function drawOverviewCharts(reports) {
		...
		reports.forEach(function(report) {
			...
			sloc.ymax = Math.max(sloc.ymax, report.complexity.aggregateAverage.sloc.physical);
			bugs.ymax = Math.max(bugs.ymax, report.complexity.aggregateAverage.halstead.bugs.toFixed(2));


			sloc.data.push({
				value: report.complexity.aggregateAverage.sloc.physical,
				label: report.info.fileShort
			});
			bugs.data.push({
				value: report.complexity.aggregateAverage.halstead.bugs.toFixed(2),
				label: report.info.fileShort
			});
